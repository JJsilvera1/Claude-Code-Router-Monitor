import { OpenRouterUsageData, OpenRouterAggregateData } from "./types";
import { clearScreen, formatCost, formatTimeAgo } from "./formatter";

function aggregateStats(data: OpenRouterUsageData[]): OpenRouterAggregateData {
    if (data.length === 0) {
        return {
            totalRequests: 0,
            totalUsage: 0,
...
function displaySummary(data: OpenRouterUsageData[], aggregated: OpenRouterAggregateData) {
    ...
    console.log(`  Total Usage: \x1b[33m${formatCost(aggregated.totalUsage)}\x1b[0m`);
    console.log(`  Total Limit: \x1b[33m${formatCost(aggregated.totalLimit)}\x1b[0m`);
    console.log(`  Credits Remaining: \x1b[33m${formatCost(data.length > 0 ? data[data.length - 1].creditsRemaining : 0)}\x1b[0m`);
    console.log();
}

function displayKeyBreakdown(data: OpenRouterUsageData[], aggregated: OpenRouterAggregateData) {
    ...
    console.log(`    Usage: \x1b[36m${formatCost(aggregated.byKeyStatus.active.usage)}\x1b[0m`);
    console.log(`    Limit: \x1b[36m${formatCost(aggregated.byKeyStatus.active.limit)}\x1b[0m`);
    console.log(`    Available: \x1b[36m${formatCost(aggregated.byKeyStatus.active.limit - aggregated.byKeyStatus.active.usage)}\x1b[0m`);
    
    if (aggregated.byKeyStatus.disabled.count > 0) {
        console.log(`  \x1b[31mDisabled Keys (${aggregated.byKeyStatus.disabled.count}):\x1b[0m`);
        console.log(`    Usage: \x1b[36m${formatCost(aggregated.byKeyStatus.disabled.usage)}\x1b[0m`);
        console.log(`    Limit: \x1b[36m${formatCost(aggregated.byKeyStatus.disabled.limit)}\x1b[0m`);
    }
    console.log();
}

function displayIndividualKeys(data: OpenRouterUsageData[]) {
    if (data.length === 0) {
        return;
    }
    ...
    sortedKeys.forEach(key => {
        ...
        console.log(`    Usage: \x1b[36m${formatCost(key.usage)}\x1b[0m / \x1b[36m${formatCost(key.limit)}\x1b[0m (\x1b[36m${usagePercent.toFixed(1)}%\x1b[0m)`);
        console.log(`    Remaining: \x1b[36m${formatCost(remaining)}\x1b[0m`);
        console.log(`    ${progressBar}`);
        console.log();
    });
}

function displayUsageHistory(data: OpenRouterUsageData[]) {
    if (data.length < 2) {
        return;
    }
    ...
    recentData.forEach((entry, index) => {
        ...
        const changeStr = usageChange > 0 ? ` (+${formatCost(usageChange)})` : '';
        
        console.log(
            `  \x1b[2m[${formatTimeAgo(entry.timestamp)}]\x1b[0m` +
            ` \x1b[36m${formatCost(entry.totalUsage)}\x1b[0m` +
            ` used of \x1b[36m${formatCost(entry.totalLimit)}\x1b[0m` +
            ` \x1b[33m${formatCost(entry.creditsRemaining)}\x1b[0m left` +
            `${changeStr}`
        );
    });
    console.log();
...
function displayAlerts(data: OpenRouterUsageData[], aggregated: OpenRouterAggregateData) {
    if (data.length === 0) {
        return;
    }
    ...
    if (latest.creditsRemaining < 10) {
        hasAlerts = true;
        console.log(`\x1b[1m\x1b[31m⚠️  LOW CREDITS: Only ${formatCost(latest.creditsRemaining)} remaining\x1b[0m`);
    }
    
    // Check for any keys approaching their individual limits
    const nearLimitKeys = latest.keys.filter(key => 
        !key.disabled && 
        key.limit > 0 && 
...
let lastData: OpenRouterUsageData[] = [];
let lastUpdateTime: number = 0;

export async function updateDisplay(data: OpenRouterUsageData[]) {
